package com.frame.saeasyandroids.http;
...
public class HttpClientFactory {
	...
	private static class SSLSocketFactoryEx extends SSLSocketFactory {
		...
		public SSLSocketFactoryEx(KeyStore truststore)
			...
			// X509TrustManager此接口的实例管理使用哪一个 X509
			// 证书来验证远端的安全套接字。决定是根据信任的证书授权、证书撤消列表、在线状态检查或其他方式做出的。
			TrustManager tm = new X509TrustManager() {
				...
				public void checkClientTrusted(
					...
					// 给出同位体提供的部分或完整的证书链，构建到可信任的根的证书路径，并且返回是否可以确认和信任将其用于基于验证类型的客户端
					// SSL 验证。
				}
				...
				public void checkServerTrusted(
						...
						String authType)
						throws java.security.cert.CertificateException {
					// 给出同位体提供的部分或完整的证书链，构建到可信任的根的证书路径，并且返回是否可以确认和信任将其用于基于验证类型的服务器
					// SSL 验证。
				}
			};
